...
Dog::Dog(Eend::Point2D position, Eend::Scale2D scale, float speed, Terrain* terrain)
    : _bodyId(Eend::Entities::boards().insert(std::filesystem::path("dog/boards/walk"))),
      _position(position), _speed(speed), _terrain(terrain), _time(0.0f) {
    Eend::Entities::boards().getRef(_bodyId)->setScale(scale);
    Eend::Entities::boards().getRef(_bodyId)->setPosition(
        Eend::Point(position.x, terrain->heightAtPoint(position), position.y));
}

Dog::~Dog() { Eend::Entities::boards().erase(_bodyId); }
...
void Dog::update(float dt, Eend::Point2D approachPoint) {
    ...
    Eend::Entities::boards().getRef(_bodyId)->setFlip(difference.x < 0.0f);
    ...
    if (_time > DOG_ANIM_INCREMENT_TIME) {
        _time = 0;
        Eend::Entities::boards().getRef(_bodyId)->nextStripIdx();
    }
