...
Duck::Duck()
    ...
    Eend::Entities::BoardBatch::getRef(_headId)->scale = Eend::Scale2D(3.5f, 3.5f);
}
...
void Duck::setPosition(Eend::Point position) {
    ...
    bodyRef->position = Eend::Point(position.x - 0.5f, position.y + 0.08f, position.z);
    headRef->position = Eend::Point(position.x, position.y + 3.00f, position.z);
}

void Duck::setRotation(float x, float y) {
    Eend::Statue* bodyRef = Eend::Entities::StatueBatch::getRef(_bodyId);
    bodyRef->rotation = Eend::Rotation(x, y);
};
