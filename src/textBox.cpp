...
TextBox::TextBox(
    ...
    : duration(seconds), clickToContinue(clickToContinue),
      _arrow(Eend::Entities::panels().insert(std::filesystem::path("textbox/arrow"))),
      _background(Eend::Entities::panels().insert(std::filesystem::path("textbox/background"))),
      _thumbnail(Eend::Entities::panels().insert(std::filesystem::path("textbox/thumbnail"))),
      _font(font), _text(font, text, Eend::Point(300.0f, 750.0f, 0.0f), 50.0f, 800.0f) {

    Eend::Panel* backgroundRef = Eend::Entities::panels().getRef(_background);
    Eend::Panel* thumbnailRef = Eend::Entities::panels().getRef(_thumbnail);
    Eend::Panel* arrowRef = Eend::Entities::panels().getRef(_arrow);
...
TextBox::~TextBox() {
    Eend::Entities::panels().erase(_background);
    Eend::Entities::panels().erase(_thumbnail);
    Eend::Entities::panels().erase(_arrow);
}
...
void TextBoxQueue::update() {
    if (_textBox) {
        if (_continue) {
            Eend::Panel* arrowRef = Eend::Entities::panels().getRef(_textBox->_arrow);
